...
def go_callback(preset_w=False, preset_h=False, mobile=False):
    if mobile:
        ...
        w_offset, h_offset = driver.execute_script("""
            ...
            """)
        w = str(int(preset_w) + int(w_offset))  # offset required to correctly resize window
        h = str(int(preset_h) + int(h_offset))  # offset required to correctly resize window
        ...
        w_offset, h_offset = driver.execute_script("""
            return [window.outerWidth - window.innerWidth,
                window.outerHeight - window.innerHeight];
            """)
        w = str(int(width.get()) + int(w_offset))  # offset required to correctly resize window
        h = str(int(height.get()) + int(h_offset))  # offset required to correctly resize window
